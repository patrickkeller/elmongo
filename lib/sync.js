...
module.exports = function (schema, options, cb) {
  ...
  var uri = (helpers.makeIndexUri(options)).toLowerCase();
  ...
  async.series({
    ...
    createIndex: function (next) {
        ...
        helpers.backOffRequest(reqOpts, function (err, res, body) {
          ...
          // if (!helpers.elasticsearchBodyOk(body) || (helpers.elasticsearchIndexExists(body))) {
    ...
    populateIndex: function (next) {
      ...
      docStream.on('data', function (doc) {
        ...
        if (commandSequence.length === BATCH_SIZE) {
          ...
          exports.bulkIndexRequest(commandSequence, options, function (err) {
            if (err) {
              return next(err);
            }
 ...
 * @api private
 */
exports.bulkIndexRequest = function (commandSequence, options, callback) {
  if (!commandSequence.length) {
    return callback();
  }
  ...
  var bulkIndexUri = helpers.makeBulkIndexUri(options)
